...
from conan.tools.files import copy, get, save, rmdir, replace_in_file, collect_libs
...
class LibwebsocketsConan(ConanFile):
    ...
    def validate(self):
        ...
        if self.options.shared and self.settings.compiler == "gcc" and compiler_version < "5":
        ...
        if Version(self.version) <= "4.0.15" and self.settings.compiler == "apple-clang" and compiler_version >= "12":
        ...
        if is_msvc(self) and compiler_version < 16 and Version(self.version) >= "4.3.2":
    ...
    def generate(self):
        ...
        if is_msvc(self) and Version(str(self.settings.compiler.version)) == 16:
